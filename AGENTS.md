# Python 编程约束（AGENTS.md）

本文件约束本目录及子目录内新编写或修改的全部 Python 代码，包括脚本、应用、库、测试、Notebook、Markdown 示例和辅助工具。用户最新明确要求优先于本文。修改前读取本文件及目标目录沿途适用的 `AGENTS.md`。

## 第一原则：默认输入合法，禁止防御性编程

**默认输入合法，只保留真实逻辑。** 所有代码默认输入合法、环境满足已说明的条件，直接执行操作。不为假设中的非法输入增加类型、范围、空值、字段或文件存在性检查，不机械增加 `main()`、入口保护和异常包装；发生异常时让原始异常暴露。只有当前功能确实存在不同处理情况，例如分类计算、筛选、算法停止或状态变化，才使用必要分支。条件判断、异常处理、校验或入口机制本身是当前教学目标时，展示该目标所需代码。实际恢复、错误转换及资源关闭也须有当前任务中的具体用途，不能仅凭“接口契约”“外部输入”或“工程实践”增加防护。测试与验收保留核对当前目标所需的断言和预期失败检查，失败必须如实报告。不得用三元表达式、布尔短路、默认值、异常抑制或批量忽略警告变相兜底。判断依据是：合法输入下，该处理是否仍为当前功能、教学或检查目标所必需。

### `if`、`try` 的使用条件

**默认禁止添加 `if`、`elif`、`try`；只有符合上述第一原则、且能说明当前具体用途时才允许使用。** 不得仅凭“可能出错”“外部输入”“接口契约”或“一般工程实践”增加分支、校验或异常捕获。

| 允许情形 | 必须满足的条件 |
| --- | --- |
| 真实功能分支 | 合法输入下仍有不同处理情况，如分类计算、筛选、算法停止或状态变化；分支直接实现当前功能。 |
| 机制教学 | 条件判断、异常处理、校验或入口机制本身就是当前教学目标；仅展示该目标所需代码。 |
| 恢复、错误转换或资源关闭 | 当前任务确有具体用途；捕获范围和处理动作只覆盖该用途，没有处理必要就让原始异常传播。 |
| 测试与验收 | 断言或预期失败检查直接核对当前目标；检查失败必须如实报告。 |

不得用条件表达式、布尔短路、默认值、异常抑制或批量忽略警告绕过上述限制。不得为假设中的类型错误、越界、空值、缺失字段或不存在的文件添加防护。合法输入本来就包含的不同情况，按真实功能处理。

不机械添加 `main()` 或 `if __name__ == "__main__":`。只有当前确需区分导入与直接执行的行为，或入口机制本身就是教学目标时，才采用相应入口结构。

写入或保留这些处理前，必须能回答：**合法输入下，该处理是否仍为当前功能、教学或检查目标所必需？** 无法说明具体用途时，删除该处理，直接执行操作。

### 与下文 Python 规范的关系

下文沿用 Python 规范的正文、示例和来源，并按用户明确要求调整。语言与 API 的实际行为不因本约束改变；涉及是否添加分支、校验、异常处理或入口结构的实现取舍，统一服从第一原则。

第 5 节的输入校验、第 6 节的异常处理、第 8.2 节的外部数据处理及第 9 节的程序入口，均须先满足上述使用条件。它们说明相应机制与适用场景，不授予为当前任务额外增加防护的权限。第 11 节的测试建议也不要求为任务之外的假设错误扩展实现。

## Python 编程规范

> 阅读定位：以下保留语言或平台规则、代码风格和一般工程建议。校验、异常、入口等章节是在讲相应机制；其写法不自动成为其他代码的必加结构。一般工程建议不能作为违反第一原则的依据。下列版本与来源核查日期沿用原文，不代表本次合并重新核查了外部来源。
...
| PyPA 与 pip 文档 | Python 打包规范及工具维护者指南 | 项目元数据、依赖声明、可重复安装与版本维护 |
...
**工程建议**用于解决特定的维护、测试或交付问题。采用哪种格式化器、哪些检查进入持续集成，应结合项目类型决定。下文用“建议”“可采用”“适用于”等措辞保留这种选择空间。
...
类型标注用于静态检查、编辑器辅助与接口说明；Python 运行时不会自动强制执行函数和变量的标注。本项目默认输入合法，禁止仅因类型标注不执行运行时检查，或输入来自文件、网络、命令行而增加防护性校验。
...
**实现约束：** 是否编写校验代码，按第一原则判断。对象可变、数据再次来自外部、保存操作或接口声明了取值范围，都不能单独作为增加校验的理由。不得仅为校验而进行“对象 → 字典 → 新对象”的往返转换。

校验机制本身是当前教学目标时，只展示该目标所需的检查；测试与验收只保留核对当前目标所需的断言和预期失败检查。默认输入合法是本项目的实现前提，不表示类型标注、`cast()` 或内部命名约定已经验证了数据。
...
- `cast()` 表达作者对类型的断言，不转换对象，也不验证对象的内容；不能把它当作运行时校验，也不能因它不执行校验而额外增加防护。

## 6 异常处理与资源管理

### 6.1 在能处理问题的位置捕获异常

默认不捕获异常，让原始异常直接传播。只有符合第一原则、当前任务确需恢复、错误转换或资源关闭，或异常处理本身就是当前教学目标时，才编写相应的 `try` 结构。需要捕获时，尽量捕获具体异常，并把 `try` 范围缩到对应操作。

**实现约束：** 文件读写或解析可能失败、代码位于应用最外层、希望提供友好提示或记录日志，都不能单独作为添加异常捕获的理由。不得仅为再抛出同一异常或让多个层级重复记录同一次失败而增加捕获。是否保留处理，按第一原则核对当前具体用途，不按分支或捕获的数量判断。

裸 `except:` 会连同 `KeyboardInterrupt`、`SystemExit` 一起捕获。不得为应用最外层机械添加裸 `except:` 或 `except Exception` 兜底，也不得捕获后无条件忽略异常。

转换异常时，用 `raise ... from error` 保留原始原因。普通自定义错误继承 `Exception` 或适当的内置异常；异常类别应帮助调用者判断问题类型。

下面的示例专门演示范围校验与异常链，属于第一原则允许的机制教学。示例将 0～100 设为有效范围，用 `if` 演示范围检查，用 `try/except` 与 `raise ... from` 演示异常转换并保留原始原因。普通整数转换任务默认输入合法，直接调用 `int(text)`；不得仅因接口声明了范围而照搬这些防护代码。
...
本项目仅在核对当前目标的测试与验收，或断言机制本身的教学中使用 `assert`，不把它作为输入防护。优化模式可能移除断言，不应依赖它完成用户输入、权限或业务规则校验。
...
- 不反序列化不可信的 `pickle` 数据，其加载过程可能执行代码。
...
需要独立的程序入口文件时，统一命名为 `main.py`，不使用 `__main__.py`。这是本项目的文件命名约定，不规定入口文件所在的仓库目录。
...
PyPA 与 pytest 的安装指南支持分别测试开发源码和安装后的包。据此，本文建议对需要分发的项目，在交付前检查实际构建产物：
...
测试围绕当前功能与检查目标编写，核对返回值、必要副作用和预期失败。Google 工程评审指南要求检查测试是否有意义、实现出错时是否真的会失败。

按第一原则，优先核对合法输入下的实际行为；空输入、边界值只有属于当前功能的合法输入，或本身就是当前教学、测试与验收目标时，才纳入相应检查。预期失败可以核对操作直接产生的原始异常，不要求被测代码额外校验、捕获或转换异常。不得为了扩充错误路径覆盖而给实现添加防护；检查失败必须如实报告。
...
10. **Google Engineering Practices｜第 1、4～6、11 节。** [评审应关注的设计、复杂度、测试和文档](https://google.github.io/eng-practices/review/reviewer/looking-for.html)、[复杂度与过度工程](https://google.github.io/eng-practices/review/reviewer/looking-for.html#complexity)、[评审标准与个人偏好的边界](https://google.github.io/eng-practices/review/reviewer/standard.html)。第 4.1 节标为“工程整理”的函数拆分建议，是本文结合这些评审原则提出的建议，不是 Google 原文逐项列出的规则。第 5.1、6.1 节标为“实现约束”的要求来自本项目第一原则，不作为 Google 或 Python 官方的通用要求。工程检查的组合与交付建议另结合 PyPA 指南整理。
