"""与 Java ``ErrorCode`` / ``BizException`` 对齐的错误契约。"""

from enum import Enum


class ErrorCode(Enum):
    """全量错误码。``code`` 与 ``message`` 与 Java 枚举逐项一致。"""
    ...
    AGENT_PLATFORM_REPO_LOCKED = (
        "14015",
        "平台智能体默认拥有全部仓库的只读权限，仓库配置不可修改",
    )
    ...
    EXECUTOR_LAUNCH_CONFIG_INCOMPLETE = (
        "17007",
        "执行器启动配置不完整，请先保存启动配置后再生成命令",
    )
    EXECUTOR_LAUNCH_CONFIG_OVERRIDE_REJECTED = (
        "17008",
        "启动配置以数据库为准，不支持临时覆盖；"
        "请调用 autowonder.update_executor_launch_config 修改后再生成命令",
    )
    ...
    DEACTIVATION_BLOCKED_BY_SOLE_ADMIN = (
        "29003",
        "您是某工作空间的唯一管理员，请先转让管理权后再申请注销",
    )
    ...
    ENVIRONMENT_VARIABLE_REFERENCE_INVALID = (
        "33008",
        "数字员工版本引用的环境变量已不存在，请先移除引用",
    )
...
class WorkspaceAccessDenied(Exception):
    ...
    def __init__(self, current: str, required: str, action: str) -> None:
        self.current = current
        self.required = required
        self.action = action
        super().__init__(ErrorCode.WORKSPACE_ACCESS_INSUFFICIENT.message)
